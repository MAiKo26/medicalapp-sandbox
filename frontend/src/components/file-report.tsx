...
function FileReport() {
  ...
  async function extractDetails(): Promise<void> {
    ...
    const response = await fetch(`${env.VITE_API_URL}/ai/analyze-report`, {
      method: "POST",
      credentials: "include",
      headers: {
    ...
    <div className="grid w-full items-start gap-6 overflow-auto p-4 pt-0">
      <fieldset className="relative grid gap-6 rounded-lg border p-4">
        ...
        {isLoading && (
          <div
            className={
              "absolute z-10 flex h-full w-full flex-row items-center justify-center rounded-lg bg-card/90"
            }
          >
            extracting...
          </div>
